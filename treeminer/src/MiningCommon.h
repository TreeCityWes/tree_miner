...
// Stockpile mode: when > 0, every new batch mines at exactly this memory cost regardless of
...
// difficulty (income keeps flowing) while the remaining streams mine at the lock. 0 = every
// stream mines the lock (whole-card stockpile). Ignored while the lock is off.
